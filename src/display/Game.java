...
public class Game extends JPanel implements KeyListener{
	...
	private static int speed = 10,jengSize = 60 ,waveHeight = 50;
	private static int base=400,xStart = 1000;
	private long point = 0,lastPress=0;
	
	private Jeng jeng = new Jeng(100,base-50);
...
//--------------------Cloud--------------------------------
	...
	public void paint(Graphics g) {
			try {
				...
				drawDogHealth(g2);
				g2.drawImage(jeng.getImage(),jeng.x,jeng.y,jengSize,jengSize, null);
				//----Wave----
	...
	private void drawDogHealth(Graphics2D g2) {
		try {
			...
			g2.drawLine(60, 30,60+jeng.health,30);	
			g2.setColor(Color.white);
	...
	private void drawWave(Wave wave,Graphics2D g2) {
			...
			if(Event.checkHit(jeng,wave,jengSize,waveHeight)){
					g2.setColor(new Color(241, 98, 69));
					g2.fillRect(0, 0,1000,1000);			
					jeng.health-=20;
					if(jeng.health<=0) {
						display.endGame(this.point);
						jeng.health = new Jeng().health;
						this.point = 0;	
					}
			}
	}
